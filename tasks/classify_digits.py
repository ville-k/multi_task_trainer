...
def create_head(head_name, out, labels, class_count):
    """Create a digit classification head.
...
def preprocess(example):
    """Preprocess examples to match model input layer shape."""
    image = example['image']
...
def create_task():
    """
    Create task that classifies numbers based on the MNIST dataset.

    Returns:
        (Task): Task with an digit classifier model head.
    """
    name = "digit_classifier"
    mnist = DatasetSets.load_mnist()

    def build_head(body_output, labels): return create_head(
        name, body_output, labels, len(mnist.labels))

    return Task(
        build_head=build_head,
        training_set=mnist.train.shuffle(buffer_size=1000).map(preprocess),
        test_set=mnist.test.shuffle(buffer_size=1000).map(preprocess),
    )
